...
using BMT.CMS.ExcelProcessor.CustomValidation;
using BMT.CMS.ExcelProcessor.CustomTemplate;

namespace BMT.CMS.ExcelProcessor.Services
{
				...
				for (int row = 2; row <= rowCount; row++)
				{
					...
					for (int columnIndex = 1; columnIndex <= templateStructures.Length(); columnIndex++)
					{
						...
						try
						{
							...
							{
								rowValue.Add(item.Name, Convert.ChangeType(Format(item.Type!, cellValue), item.Type!));
							}
